...
fontfamily: mathpazo
fontsize: 12pt
header-includes: \usepackage{tabularx} \usepackage{float,booktabs,longtable,multirow} \usepackage{graphicx,colortbl,xcolor,wrapfig}\usepackage{mathptmx,fixltx2e} \usepackage[font=small,skip=0pt]{caption}
csl: ecology.csl
...
In the intro, write about microbial community as an envirnomental variable
...
\textbf{H\textsubscript{2}}: The idiosyncratic responses of species to environmental gradients may be correlated with their functional traits. This pattern may arise if the functional traits are indicative of species' environmental preference along the gradient sampled (Figure \ref{fig:ch1-hypotheses}a). 
...
**Demographic response**: In 2015-2016, I planted seeds of 17 annual plant species (Table \ref{tab:table-species}) in a setup that allowed me to parameterize a simplified annual plant popuation growth model. Specifically, the population growth rate of species $j$ at site $x$ is modeled as a function of the germination rate $g_{j,x}$, a global seed-bank survival rate for each species $s_j$, low-density seed production $\lambda_{j,x}$, species response to competitors $r_{j,x}$, and the competitive neighborhood $\eta_x$: 
...
**Abiotic environmental variation**: We characterized various environmental variables at each of our experimental sites during the 2015-16 growing season. Soil physical and chemical properties were analysed by A&L Western Labs. Air temperature at each of the plots was measured with temperature data loggers (iButtons) that were placed inside PVC tubing to shield from solar radiation. The gravimetric water content was measured for each plot on 1 April 2016 (corresponding to the peak growing season for the majority of species that have an earlier phenology) and on 1 May 2016 (corresponding to peak growing season for a suite of later-phenology species).
...
**Analyses**: I propose to assess the influence of functional traits on the demographic responses of species to environmental gradients with a generalized linear mixed modeling (GLMM) framework modified from [@Jamil_2012]. This framework, which was developed under the context of the fourth-corner problem [@Legendre_1997], models the likelihood of species presence across a landscape as a function of species traits, environmental properties, and crucially, an interaction term between the two: 
\[  
logit(p_{i,j}) = a_0+\underbrace{a_1z_j}_{trait\ effect}+\overbrace{b_0x_i}^{environment\ effect}+\underbrace{b_1z_jx_i}_{interaction}+\overbrace{\epsilon_{\alpha j} + \epsilon_{\beta i} + \gamma_i}^{error\ terms}
\]

In this framework, the contribution of a trait to species' differences in the slope parameter $\beta_j$ can be quantified as a function of the variances $\hat\sigma_b$ between a model that includes a trait:environment term and one that lacks this interaction:

$$C_\beta = 1-\frac{\hat\sigma^2_{\beta,\ no\ interaction\ term}}{\hat\sigma^2_{\beta,\ with\ interaction\ term}}$$

@Jamil_2012 demonstrate that this framework can be extended to model abundance data, and I propose to modify it to model the experimentally estimated metrics of species performance. 


### Preliminary results, implications, and further analyses
A large proportion of the species pairs had uncoupled demographic responses- in other words, most species pairs did not respond in the same way to the multivariate changes in environment across sites (Figure \ref{fig-ch1-result1})


-- Robin Snyder talk- propose to do a simulation of how strong we expect trait-demography relationship to be accounting for "luck" when we know true strength
