...
**Demographic variation**: In 2015-2016, we planted seeds of 17 annual plant species (Table 1) at 24 plots in a setup that allowed me to parameterize a simplified annual plant population growth model. In this model, the population size of a species $j$ at a site $x$ grows according to it seed bank survival rate ($s_j$) and its intrinsic germination and seed production response at each site ($g_{j,x}$ and $\lambda_{j,x}$ respectively), but is reduced by how much competition it faces at each site ($\eta_x$) and how sensitive it is to competitors ($r_{j,x}$): 
...
NEEDS SOME SENTENCES ABOUT THE FIELD HAVING FREAKOUTS ABOUT ITV- DO WE NEED TO MEASURE ALL INDIVIDUALS?!??!?!?!
